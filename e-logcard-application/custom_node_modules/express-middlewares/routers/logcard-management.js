...
router.put("/aircrafts/remove/:id", function (req, res, next) {
	enrollUser(req,function(enrolledUser){
		var id = req.params.id;
...
router.put("/aircrafts/scrapp/", function (req, res, next) {
	enrollUser(req,function(enrolledUser){
		var id = req.body.id;
		var vDate = format.asString("yyyy/MM/dd hh:mm:ss", new Date());
		logcardManager.scrappAircraft(enrolledUser, id, vDate).then(function () {
            console.log("Aircraft scrapped with success");
            res.status("200").send(true);
        });
	});
...
router.put("/assemblies/scrapp/", function (req, res, next) {
	enrollUser(req,function(enrolledUser){
		var id = req.body.id;
		var vDate = format.asString("yyyy/MM/dd hh:mm:ss", new Date());
		logcardManager.scrappAssembly(enrolledUser, id, vDate).then(function () {
            console.log("Assembly scrapped with success");
            res.status("200").send(true);
        });
	});
