...
router.get("/parts/:id", function (req, res, next) {
	enrollUser(req,function(enrolledUser){
		var id = req.params.id;
		let parts = logcardManager.getOnePartsDetails(enrolledUser, id).then(function (parts) {
            res.status(200).type('application/json').send(parts);
        });
	});
});

router.get("/partsNoAssembly/", function (req, res, next) {
	enrollUser(req,function(enrolledUser){
...
router.put("/aircrafts/transferRespo/:id", function (req, res, next) {
	enrollUser(req,function(enrolledUser){
		var responsible = req.body.responsible;
		var id = req.params.id;
		var vDate = format.asString("yyyy/MM/dd hh:mm:ss", new Date());

		logcardManager.AcRespoTransfer(enrolledUser, id, responsible, vDate).then(function () {
...
router.get("/assembliesNoAircraft", function (req, res, next) {
	enrollUser(req,function(enrolledUser){
        let parts = logcardManager.getAllAssembliesWithoutAC(enrolledUser).then(function (parts) {
            res.status(200).type('application/json').send(parts);
        });
	});
});
...
router.put("/assemblies/transferRespo/:id", function (req, res, next) {
	enrollUser(req,function(enrolledUser){
		var responsible = req.body.responsible;
		var id = req.params.id;
		var vDate = format.asString("yyyy/MM/dd hh:mm:ss", new Date());
		logcardManager.AssembRespoTransfer(enrolledUser, id, responsible, vDate ).then(function () {
		console.log("Responsible Transfer succeed");
		res.status("200").send(true);
		});
	});
...
router.get("/List/modifications", function (req, res, next) {
	let modifications = [
			{"label": "Service Bulletin", "value": "SB"},
