...
router.get("/parts/:typ", function (req, res, next) {
	enrollUser(req,function(enrolledUser){
	var typ = req.params.typ;
		let parts = logcardManager.getAllPartsDetails(enrolledUser, typ).then(function (parts) {
...
router.get("/parts/:typ/:id/historic", function (req, res, next) {
	enrollUser(req,function(enrolledUser){
		var id = req.params.id;
		var typ = req.params.typ;
        let parts = logcardManager.getPartDetails(enrolledUser, typ, id).then(function (parts) {
            res.status(200).type('application/json').send(parts);
        });
	});
});
